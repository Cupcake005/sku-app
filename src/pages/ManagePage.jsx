...
const ManagePage = () => {
  ...
  const handleSaveProduct = async (formData, isVariantMode = false) => {
    ...
    // PASTIKAN TIDAK ADA 'unit' DI SINI
    const payload = {
        sku: formData.sku,
  ...
  // --- 5. EXPORT CSV (Tetap Pakai Format Lama agar Kompatibel) ---
  const handleExport = () => { 
      if (products.length === 0) return alert("Data kosong!");
      
      // KITA PERTAHANKAN KOLOM UNIT DI HEADER AGAR URUTAN TIDAK RUSAK
      const header = "Category,SKU,Unit,Items Name (Do Not Edit),Brand Name,Variant name,Price,Wholesale Price";
      
      const rows = products.map(item => {
        const category = `"${item.category || ''}"`;
        const sku = `"${item.sku || ''}"`; 
        const unit = `"-"`; // ISI DENGAN "-" AGAR EXCEL TIDAK BERGESER
  ...
  // --- 6. IMPORT CSV (FIX: HAPUS UNIT DARI PAYLOAD) ---
  ...
  const processImport = async (csvText) => { 
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            // Kita asumsikan format Excel masih yang LAMA (8 kolom termasuk Unit)
            // Kolom 0: Category
            // Kolom 1: SKU
            // Kolom 2: Unit (KITA ABAIKAN, TAPI JANGAN GESER INDEX)
            // Kolom 3: Nama Barang
            // ... dst
            ...
            if (columns.length >= 7) {
                ...
                // Index 2 adalah Unit, kita LEWATI saja.
                const item_name = clean(columns[3]); // Index 3 tetap Nama Barang
                ...
                if (item_name) { 
                    // SAAT MEMBUAT OBJEK, JANGAN MASUKKAN 'unit'
                    dataToInsert.push({ 
                        user_id: user.id, 
                        category, 
                        sku: String(sku), 
                        // unit: ..., // <--- HAPUS BARIS INI TOTAL
                        item_name, 
                        brand_name, 
                        variant_name, 
                        price,
    ...
    <div className="pb-24 relative">
      <div className="bg-white p-4 rounded-lg shadow-md min-h-[80vh]">
        ...
        <div className="flex flex-col gap-2 mb-6">
          ...
          <button onClick={handleOpenAdd} className="w-full bg-indigo-600 text-white py-2 rounded-lg font-bold text-sm flex items-center justify-center gap-2 hover:bg-indigo-700 shadow"><Plus size={18} /> Tambah Data Manual</button>
        ...
        <div className="relative mb-4">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Cari Nama atau SKU..." className="w-full pl-10 pr-12 py-3 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none"/>
      ...
      <ProductModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setSearchParams({}); }} product={currentProduct} onSave={handleSaveProduct} onScanClick={() => { setIsModalOpen(false); setShowScanner(true); }} allProducts={products} />
    </div>
  );
};
