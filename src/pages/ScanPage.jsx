...
const ScanPage = () => {
  ...
  const executeSearch = async (queryText) => {
      ...
      if (results.length < 5) {
          try {
              ...
              if (!error && data) {
                  // Merge results (hindari duplikat)
  ...
  const handleScan = async (sku) => {
    playBeep();
    setSearchQuery(sku); // Isi kolom search
    await executeSearch(sku); // Jalankan pencarian
  };
  ...
  const handleSaveProduct = async (formData, isVariantMode = false) => {
    ...
    const payload = {
        ...
        price: parseFloat(formData.price) || 0,
        wholesale_price: parseFloat(formData.wholesale_price) || 0,
        // Hapus unit dari sini agar tidak error column not found
    };
    ...
    if (error) {
        ...
        if (data) {
            // Update local state & search results agar langsung muncul
            ...
            <div className="flex justify-between items-center mb-2">
             ...
             {loading ? (
                 ...
                 <div className="space-y-4 pb-20 pt-2"> 
                    {searchResults.map((item) => {
                      // Logic Highlight: Cek apakah SKU sama persis dengan yang dicari
